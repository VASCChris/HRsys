...
function addEmp(data){
	...
	if("離職"==status){
		...
		bodyHTML += '<td><select id="status' + id +'"><option value="'+data.status+'">'+ data.status +'</option><option value="在職">在職</option><option value="離職">離職</option></select></td>';
		bodyHTML += '<td><button type="button" name="send" style="width: 45px;" onclick="update('+ id +')">修改</button></td></tr>';
